...
widgetKeyBoard::widgetKeyBoard(bool embeddedKeyboard, QWidget *activeForm, QWidget *parent) : QWidget(parent), m_nextInput(NULL), m_activeWindow(activeForm),
        m_currentTextBox(NULL), m_embeddedKeyboard(embeddedKeyboard), m_echoMode(false), m_zoomFacilityEmbedded(false), m_enablePasswordEcho(false), m_clipboard(QGuiApplication::clipboard())
{        
...
//
    ...
    if (NO_SPECIAL_KEY(newKey) == false) {
        if (IS_RETURN(newKey) == true || IS_TAB(newKey) == true) { // trattasi di TAB, si sposta alla text successiva
            ...
            qDebug()<<newKey<<"i'm here"<<tmpReceiptString;//xin hao cao
        }
...
//    this->move(20, QGuiApplication::screens()[0]->availableGeometry().height() - this->height() - 200);
    this->setGeometry(10,230,1000,300);
...
void widgetKeyBoard::focusThis(QLineEdit *control)
{
    if (this->m_activeWindow != NULL) {
        ...
        ;// Non \E8 attiva nessuna finestra su cui operare
    }
}
...
void widgetKeyBoard::createKeyboard(void)
{
    ...
    if (widgetKeyBoard::m_created == true) // tastiera gi\E0  creata: esce
		return;
    widgetKeyBoard::m_created = true; // segnala che \E8 stata creata la tastiera
	//
    // stampa tasti numerici:
//	tmpLayout->addWidget(createNewKey("\\"));
    for (short i = 49; i <= 57; i++) {
        tmp = createNewKey(QChar(i));
        tmp ->setMaximumWidth(78);
        tmp ->setMinimumWidth(78);
...
//    else
//        tmpStyle = QString("<---");
    tmp = createNewKey(QString("delete"));
    tmp->setMaximumWidth(tmp->maximumWidth()+35);
    //qDebug()<<tmp->maximumWidth();
    tmp->setMinimumWidth(tmp->minimumWidth()+35);
    //qDebug()<<tmp->minimumWidth();
    tmpLayout->addWidget(tmp);
...
//    tmpLayout->addWidget(tmp);
...
#endif
    tmpVLayout->insertLayout(0, tmpLayout,0);
	//
    ...
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("Q"));
    tmp ->setMaximumWidth(60);
    tmp ->setMinimumWidth(60);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("W"));
    tmp ->setMaximumWidth(60);
    tmp ->setMinimumWidth(60);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("E"));
    tmp ->setMaximumWidth(60);
    tmp ->setMinimumWidth(60);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("R"));
    tmp ->setMaximumWidth(60);
    tmp ->setMinimumWidth(60);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("T"));
    tmp ->setMaximumWidth(60);
    tmp ->setMinimumWidth(60);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("Y"));
    tmp ->setMaximumWidth(60);
    tmp ->setMinimumWidth(60);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("U"));
    tmp ->setMaximumWidth(60);
    tmp ->setMinimumWidth(60);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("I"));
    tmp ->setMaximumWidth(60);
    tmp ->setMinimumWidth(60);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("O"));
    tmp ->setMaximumWidth(60);
    tmp ->setMinimumWidth(60);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("P"));
    tmp ->setMaximumWidth(60);
    tmp ->setMinimumWidth(60);
    tmpLayout->addWidget(tmp);
...
//    tmpLayout->addWidget(createNewKey(tr("-")));
    ...
    tmp->setMaximumWidth(tmp->maximumWidth() * 2 + 30);
    tmp->setMinimumWidth(tmp->minimumWidth() * 2 + 30);
    if (this->isEmbeddedKeyboard() == true)
        tmpStyle = QString(EMBEDDED_KEYBOARD);
    tmp->setStyleSheet(QString(DEFAULT_STYLE_BUTTON) + QString(CHANGED_BACKGROUND_BUTTON) + tmpStyle); // segnalato come attivo
    tmpLayout->addWidget(tmp);

    tmp = createNewKey(tr("A"));
    tmp ->setMaximumWidth(65);
    tmp ->setMinimumWidth(65);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("S"));
    tmp ->setMaximumWidth(65);
    tmp ->setMinimumWidth(65);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("D"));
    tmp ->setMaximumWidth(65);
    tmp ->setMinimumWidth(65);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("F"));
    tmp ->setMaximumWidth(65);
    tmp ->setMinimumWidth(65);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("G"));
    tmp ->setMaximumWidth(65);
    tmp ->setMinimumWidth(65);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("H"));
    tmp ->setMaximumWidth(65);
    tmp ->setMinimumWidth(65);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("J"));
    tmp ->setMaximumWidth(65);
    tmp ->setMinimumWidth(65);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("K"));
    tmp ->setMaximumWidth(65);
    tmp ->setMinimumWidth(65);
    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("L"));
    tmp ->setMaximumWidth(65);
    tmp ->setMinimumWidth(65);
    tmpLayout->addWidget(tmp);
...
//    tmpLayout->addWidget(createNewKey(tr("\C3 ")));
//    tmpLayout->addWidget(createNewKey(tr("ù")));
//    tmpLayout->addWidget(createNewKey(tr("@")));
    tmpLayout->insertStretch(-1, 1);
#if !defined(QT_ARCH_MACOSX)
    layoutReturn->setSpacing(10);
    ...
    tmp->setMaximumWidth(tmp->maximumWidth() +60);
    tmp->setMinimumWidth(tmp->minimumWidth() +60);
    tmp->setMinimumHeight(tmp->minimumHeight() +50);
    tmp->setMaximumHeight(tmp->maximumHeight() +50);
	tmpLayout->addWidget(tmp);
	//
...
//    tmpLayout->addWidget(tmp);
    tmp = createNewKey(tr("SHIFT"));
    tmp->setMaximumWidth(tmp->maximumWidth() * 2 + 60);
    tmp->setMinimumWidth(tmp->minimumWidth() * 2 + 60);
    tmpLayout->addWidget(tmp);
...
//    tmp->setMaximumWidth(tmp->maximumWidth() * 3 + 25);
//    tmp->setMinimumWidth(tmp->minimumWidth() * 3 + 25);
//    tmpLayout->addWidget(tmp);
    tmpLayout->insertStretch(-1, 1);
    layoutReturn->insertLayout(2, tmpLayout, 1); // inserisce la riga della "A"
	//	
...
//    tmpLayout->addWidget(tmp);
    ...
    tmp->setMaximumWidth(tmp->maximumWidth() * 15);
    tmp->setMinimumWidth(tmp->minimumWidth() * 15);
    tmpLayout->addWidget(tmp);
